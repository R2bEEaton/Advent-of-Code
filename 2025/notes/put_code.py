...
for file in os.listdir("../code"):
    ...
    r = requests.get("https://adventofcode.com/2025/leaderboard/private/view/144198.json", cookies={'session': open("../code/helpers/sess", "r").read()})
    r = json.loads(r.text)
...
```python
%s
```""" % (fname, int(fname)-1, int(fname)+1, parta, delta, partb, open("../code/day%sa.py" % fname).read(), open("../code/day%sb.py" % fname).read()))
