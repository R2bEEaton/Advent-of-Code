...
A very cool problem! Unfortunately I slept through my alarm to wake up and do Advent of Code. Maybe it's because I've had no chance at #1 in the NJIT ACM Leaderboard for days, my body was like... sleep is more important. And that's fair, I've been going to bed late every day and waking up at 7 AM.
...
3. For any graph, a property exists such that the number of paths from `u -> v` equals the sum of paths from `w -> v` for all `w` which are direct children of `u`
4. For DAGs, the prior property can be used for Dynamic Programming because:
    1. DAGs guarantee finite number of paths
    2. Topological sort gives us a processing order where all descendants are processed before any node that leads to them
...
So if we compute the number of paths between each pair, and find the product, that's our answer! After the fact, I also realized that for all paths that must include two specific intermediate nodes (when we DON'T care about the order):
...
Somehow this was most helpful reference I found, even though it's super vague. I'm happy I was able to figure out what the answerer was talking about.
...
This code is much better formatted, and removes the redundancy, utilizing topological sort to save probably hundreds of microseconds! But in all seriousness, eliminating recursion here is definitely the way to go.
